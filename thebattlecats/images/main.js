$(document).ready(function() {	
	...
	var select = $( "#XP-amount-wrapper" );
	var slider = $( "<div id='slider-XP'></div>" ).insertAfter( select ).slider({
		min: 10000,
		max: 99999,
		value: 10000,
		range: "min",
		change: function(event, ui) { 
			var sliderValue = $( "#slider-XP" ).slider( "option", "value" );				
			$('#XP-amount').html(sliderValue);
			if(sliderValue == '10000') {
				progressBar(20, $('#progressBar'));
				$('#decrease-XP').addClass('btn-disabled');
				$('.max-amount-XP').fadeOut();
			}
			else if (sliderValue == '25000') {
				progressBar(40, $('#progressBar'));
				$('#decrease-XP').removeClass('btn-disabled');
			}
				...
				$('#increase-XP').removeClass('btn-disabled');
				$('.max-amount-XP').fadeOut();
			}
			else if (sliderValue == '99999') {
				progressBar(100, $('#progressBar'));
				$('#increase-XP').addClass('btn-disabled');
				$('.max-amount-XP').fadeIn();
			}
		}        
	});	
	
	$('#increase-XP').click(function() {
	var sliderCurrentValue = $( "#slider-XP" ).slider( "option", "value" );
	  slider.slider( "value", sliderCurrentValue + 15000 );
	});

	$('#decrease-XP').click(function() {
	var sliderCurrentValue = $( "#slider-XP" ).slider( "option", "value" );
	  slider.slider( "value", sliderCurrentValue - 15000 );
	});

	function progressBarFood(percent, $element) {
		var progressBarFoodWidth = percent * $element.width() / 100;
		$element.find('div').animate({ width: progressBarFoodWidth }, 500).html(percent + "%&nbsp;");
	}
	progressBarFood(20, $('#progressBarFood'));
	var selectFood = $( "#Food-amount-wrapper" );
	var sliderFood = $( "<div id='slider-Food'></div>" ).insertAfter( selectFood ).slider({
		min: 10000,
		max: 99999,
		value: 10000,
		range: "min",
		change: function(event, ui) { 
				var sliderValueFood = $( "#slider-Food" ).slider( "option", "value" );
				$('#Food-amount').html(sliderValueFood);
					if(sliderValueFood == '10000') {
						progressBarFood(20, $('#progressBarFood'));
						$('#decrease-Food').addClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueFood == '25000') {
						progressBarFood(40, $('#progressBarFood'));
						$('#decrease-Food').removeClass('btn-disabled');
					}
					else if (sliderValueFood == '55000') {
						progressBarFood(60, $('#progressBarFood'));
					}
					else if (sliderValueFood == '70000') {
						progressBarFood(80, $('#progressBarFood'));
						$('#increase-Food').removeClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueFood == '99999') {
						progressBarFood(100, $('#progressBarFood'));
						$('#increase-Food').addClass('btn-disabled');
						$('.max-amount-points').fadeIn();
					}
				}        
	});				

	$('#increase-Food').click(function() {
		var sliderCurrentFoodValue = $( "#slider-Food" ).slider( "option", "value" );
		sliderFood.slider( "value", sliderCurrentFoodValue + 15000 );
	});

	$('#decrease-Food').click(function() {
		var sliderCurrentFoodValue = $( "#slider-Food" ).slider( "option", "value" );
		sliderFood.slider( "value", sliderCurrentFoodValue - 15000 );
	});
	...
	function loading_step() {
		...
		$('#resources-select-wrapper').fadeOut(500, function() {
			$('#processing-wrapper').fadeIn(500, function() {
				...
				var $console_message_XP_msg = $('#slider-XP').slider("option", "value");
				var $console_message_Food_msg = $('#slider-Food').slider("option", "value");			
				...
				setTimeout(function() { 
					$console_message.html("Creating packages for XP");	
					progressBarConsole(38, $('#progressBarConsole'));			
				}, 11000 );
				setTimeout(function() { 
					$console_message.html("Generating <span class='console-message-connected-item'>" + $console_message_XP_msg + "</span> XP");
					$('.console-XP-wrapper').fadeIn(500, function() {
						var $console_XP_countto = $('#slider-XP').slider("option", "value");
						$('#console-XP-value').countTo({
							from: 0,
							to: $console_XP_countto,
							speed: 2500,
							refreshInterval: 10,
				...
				setTimeout(function() {
					$console_message.html("<span class='console-message-connected-item'>" + $console_message_XP_msg + "</span> <span class='console-message-success'>XP successfully generated</span>");
					$('#console-success-confirmation-XP').fadeIn();
					$(".console-message").addClass('pulse animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){
						$(this).removeClass('pulse animated');
					});
				...
				setTimeout(function() { 
					$console_message.html("Creating packages for Food");	
					progressBarConsole(58, $('#progressBarConsole'));			
				}, 18000 );
				setTimeout(function() { 
					$console_message.html("Generating <span class='console-message-connected-item'>" + $console_message_Food_msg + "</span> Food");
					$('.console-Food-wrapper').fadeIn(500, function() {
						var $console_Food_countto = $('#slider-Food').slider("option", "value");					
						$('#console-Food-value').countTo({
							from: 0,
							to: $console_Food_countto,
							speed: 2500,
							refreshInterval: 10,
							formatter: function (value, options) {
							  return value.toFixed(options.decimals);
				...
				setTimeout(function() { 
					$console_message.html("<span class='console-message-connected-item'>" + $console_message_Food_msg + "</span> <span class='console-message-success'> Food successfully generated</span>");
					$('#console-success-confirmation-Food').fadeIn();
					$(".console-message").addClass('pulse animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){
						$(this).removeClass('pulse animated');
					});
