$(document).ready(function() {	
	...
	var select = $( "#Crystals-amount-wrapper" );
	var slider = $( "<div id='slider-Crystals'></div>" ).insertAfter( select ).slider({
		min: 100000,
		max: 999999,
		value: 100000,
		range: "min",
		change: function(event, ui) { 
			var sliderValue = $( "#slider-Crystals" ).slider( "option", "value" );				
			$('#Crystals-amount').html(sliderValue);
			if(sliderValue == '100000') {
				progressBar(20, $('#progressBar'));
				$('#decrease-Crystals').addClass('btn-disabled');
				$('.max-amount-Crystals').fadeOut();
			}
			else if (sliderValue == '250000') {
				progressBar(40, $('#progressBar'));
				$('#decrease-Crystals').removeClass('btn-disabled');
			}
				...
				$('#increase-Crystals').removeClass('btn-disabled');
				$('.max-amount-Crystals').fadeOut();
			}
			else if (sliderValue == '999999') {
				progressBar(100, $('#progressBar'));
				$('#increase-Crystals').addClass('btn-disabled');
				$('.max-amount-Crystals').fadeIn();
			}
		}        
	});	
	
	$('#increase-Crystals').click(function() {
	var sliderCurrentValue = $( "#slider-Crystals" ).slider( "option", "value" );
	  slider.slider( "value", sliderCurrentValue + 150000 );
	});

	$('#decrease-Crystals').click(function() {
	var sliderCurrentValue = $( "#slider-Crystals" ).slider( "option", "value" );
	  slider.slider( "value", sliderCurrentValue - 150000 );
	});

	function progressBarCrystals(percent, $element) {
		var progressBarCrystalsWidth = percent * $element.width() / 100;
		$element.find('div').animate({ width: progressBarCrystalsWidth }, 500).html(percent + "%&nbsp;");
	}
	progressBarCrystals(20, $('#progressBarCrystals'));
	var selectCrystals = $( "#Crystals-amount-wrapper" );
	var sliderCrystals = $( "<div id='slider-Crystals'></div>" ).insertAfter( selectCrystals ).slider({
		min: 100000,
		max: 999999,
		value: 100000,
		range: "min",
		change: function(event, ui) { 
				var sliderValueCrystals = $( "#slider-Crystals" ).slider( "option", "value" );
				$('#Crystals-amount').html(sliderValueCrystals);
					if(sliderValueCrystals == '100000') {
						progressBarCrystals(20, $('#progressBarCrystals'));
						$('#decrease-Crystals').addClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueCrystals == '250000') {
						progressBarCrystals(40, $('#progressBarCrystals'));
						$('#decrease-Crystals').removeClass('btn-disabled');
					}
					else if (sliderValueCrystals == '550000') {
						progressBarCrystals(60, $('#progressBarCrystals'));
					}
					else if (sliderValueCrystals == '700000') {
						progressBarCrystals(80, $('#progressBarCrystals'));
						$('#increase-Crystals').removeClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueCrystals == '999999') {
						progressBarCrystals(100, $('#progressBarCrystals'));
						$('#increase-Crystals').addClass('btn-disabled');
	...
	$('#increase-Crystals').click(function() {
		var sliderCurrentCrystalsValue = $( "#slider-Crystals" ).slider( "option", "value" );
		sliderCrystals.slider( "value", sliderCurrentCrystalsValue + 150000 );
	});
	...
	function loading_step() {
		...
		$('#resources-select-wrapper').fadeOut(500, function() {
			$('#processing-wrapper').fadeIn(500, function() {
				...
				var $console_message_Crystals_msg = $('#slider-Crystals').slider("option", "value");
				var $console_message_Crystals_msg = $('#slider-Crystals').slider("option", "value");			
				...
				setTimeout(function() { 
					$console_message.html("Creating packages for Crystals");	
					progressBarConsole(38, $('#progressBarConsole'));			
				}, 11000 );
				setTimeout(function() { 
					$console_message.html("Generating <span class='console-message-connected-item'>" + $console_message_Crystals_msg + "</span> Crystals");
					$('.console-Crystals-wrapper').fadeIn(500, function() {
						var $console_Crystals_countto = $('#slider-Crystals').slider("option", "value");
						$('#console-Crystals-value').countTo({
							from: 0,
							to: $console_Crystals_countto,
							speed: 2500,
							refreshInterval: 10,
				...
				setTimeout(function() {
					$console_message.html("<span class='console-message-connected-item'>" + $console_message_Crystals_msg + "</span> <span class='console-message-success'>Crystals successfully generated</span>");
					$('#console-success-confirmation-Crystals').fadeIn();
					$(".console-message").addClass('pulse animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){
						$(this).removeClass('pulse animated');
					});
