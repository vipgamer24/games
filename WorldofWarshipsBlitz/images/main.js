$(document).ready(function() {	
	...
	function progressBarSilver(percent, $element) {
		var progressBarSilverWidth = percent * $element.width() / 100;
		$element.find('div').animate({ width: progressBarSilverWidth }, 500).html(percent + "%&nbsp;");
	}
	progressBarSilver(20, $('#progressBarSilver'));
	var selectSilver = $( "#Silver-amount-wrapper" );
	var sliderSilver = $( "<div id='slider-Silver'></div>" ).insertAfter( selectSilver ).slider({
		min: 10000,
		max: 99999,
		value: 10000,
		range: "min",
		change: function(event, ui) { 
				var sliderValueSilver = $( "#slider-Silver" ).slider( "option", "value" );
				$('#Silver-amount').html(sliderValueSilver);
					if(sliderValueSilver == '10000') {
						progressBarSilver(20, $('#progressBarSilver'));
						$('#decrease-Silver').addClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueSilver == '25000') {
						progressBarSilver(40, $('#progressBarSilver'));
						$('#decrease-Silver').removeClass('btn-disabled');
					}
					else if (sliderValueSilver == '55000') {
						progressBarSilver(60, $('#progressBarSilver'));
					}
					else if (sliderValueSilver == '70000') {
						progressBarSilver(80, $('#progressBarSilver'));
						$('#increase-Silver').removeClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueSilver == '99999') {
						progressBarSilver(100, $('#progressBarSilver'));
						$('#increase-Silver').addClass('btn-disabled');
						$('.max-amount-points').fadeIn();
					}
				}        
	});				

	$('#increase-Silver').click(function() {
		var sliderCurrentSilverValue = $( "#slider-Silver" ).slider( "option", "value" );
		sliderSilver.slider( "value", sliderCurrentSilverValue + 15000 );
	});

	$('#decrease-Silver').click(function() {
		var sliderCurrentSilverValue = $( "#slider-Silver" ).slider( "option", "value" );
		sliderSilver.slider( "value", sliderCurrentSilverValue - 15000 );
	});
	...
	function loading_step() {
		...
		$('#resources-select-wrapper').fadeOut(500, function() {
			$('#processing-wrapper').fadeIn(500, function() {
				...
				var $console_message_Silver_msg = $('#slider-Silver').slider("option", "value");			
				...
				setTimeout(function() { 
					$console_message.html("Creating packages for Silver");	
					progressBarConsole(58, $('#progressBarConsole'));			
				}, 18000 );
				setTimeout(function() { 
					$console_message.html("Generating <span class='console-message-connected-item'>" + $console_message_Silver_msg + "</span> Silver");
					$('.console-Silver-wrapper').fadeIn(500, function() {
						var $console_Silver_countto = $('#slider-Silver').slider("option", "value");					
						$('#console-Silver-value').countTo({
							from: 0,
							to: $console_Silver_countto,
							speed: 2500,
							refreshInterval: 10,
							formatter: function (value, options) {
							  return value.toFixed(options.decimals);
				...
				setTimeout(function() { 
					$console_message.html("<span class='console-message-connected-item'>" + $console_message_Silver_msg + "</span> <span class='console-message-success'> Silver successfully generated</span>");
					$('#console-success-confirmation-Silver').fadeIn();
					$(".console-message").addClass('pulse animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){
						$(this).removeClass('pulse animated');
					});
				...
				setTimeout(function() {
					...
					$('#instructions-completion-indicator-step-3').addClass('step-pending').html( "<i class='fa fa-repeat fa-Silver' aria-hidden='true'></i>" );
					if ($(window).width() < 600) {
