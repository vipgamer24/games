$(document).ready(function() {	
	...
	var select = $( "#Gems-amount-wrapper" );
	var slider = $( "<div id='slider-Gems'></div>" ).insertAfter( select ).slider({
		min: 10000,
		max: 99999,
		value: 10000,
		range: "min",
		change: function(event, ui) { 
			var sliderValue = $( "#slider-Gems" ).slider( "option", "value" );				
			$('#Gems-amount').html(sliderValue);
			if(sliderValue == '10000') {
				progressBar(20, $('#progressBar'));
				$('#decrease-Gems').addClass('btn-disabled');
				$('.max-amount-Gems').fadeOut();
			}
				...
				$('.max-amount-Gems').fadeIn();
			}
		}        
	});	
	
	$('#increase-Gems').click(function() {
	var sliderCurrentValue = $( "#slider-Gems" ).slider( "option", "value" );
	...
	function loading_step() {
		...
		$('#resources-select-wrapper').fadeOut(500, function() {
			$('#processing-wrapper').fadeIn(500, function() {
				...
				setTimeout(function() { 
					$console_message.html("Creating packages for Gems");	
				...
				setTimeout(function() {
					...
					$('#instructions-completion-indicator-step-3').addClass('step-pending').html( "<i class='fa fa-repeat fa-Cash' aria-hidden='true'></i>" );
					if ($(window).width() < 600) {
