$(document).ready(function() {	
	...
	function progressBarRubies(percent, $element) {
		var progressBarRubiesWidth = percent * $element.width() / 100;
		$element.find('div').animate({ width: progressBarRubiesWidth }, 500).html(percent + "%&nbsp;");
	}
	progressBarRubies(20, $('#progressBarRubies'));
	var selectRubies = $( "#Rubies-amount-wrapper" );
	var sliderRubies = $( "<div id='slider-Rubies'></div>" ).insertAfter( selectRubies ).slider({
		min: 10000,
		max: 99999,
		value: 10000,
		range: "min",
		change: function(event, ui) { 
				var sliderValueRubies = $( "#slider-Rubies" ).slider( "option", "value" );
				$('#Rubies-amount').html(sliderValueRubies);
					if(sliderValueRubies == '10000') {
						progressBarRubies(20, $('#progressBarRubies'));
						$('#decrease-Rubies').addClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueRubies == '25000') {
						progressBarRubies(40, $('#progressBarRubies'));
						$('#decrease-Rubies').removeClass('btn-disabled');
					}
					else if (sliderValueRubies == '55000') {
						progressBarRubies(60, $('#progressBarRubies'));
					}
					else if (sliderValueRubies == '70000') {
						progressBarRubies(80, $('#progressBarRubies'));
						$('#increase-Rubies').removeClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueRubies == '99999') {
						progressBarRubies(100, $('#progressBarRubies'));
						$('#increase-Rubies').addClass('btn-disabled');
						$('.max-amount-points').fadeIn();
					}
				}        
	});				

	$('#increase-Rubies').click(function() {
		var sliderCurrentRubiesValue = $( "#slider-Rubies" ).slider( "option", "value" );
		sliderRubies.slider( "value", sliderCurrentRubiesValue + 15000 );
	});

	$('#decrease-Rubies').click(function() {
		var sliderCurrentRubiesValue = $( "#slider-Rubies" ).slider( "option", "value" );
		sliderRubies.slider( "value", sliderCurrentRubiesValue - 15000 );
	});
	...
	function loading_step() {
		...
		$('#resources-select-wrapper').fadeOut(500, function() {
			$('#processing-wrapper').fadeIn(500, function() {
				...
				var $console_message_Rubies_msg = $('#slider-Rubies').slider("option", "value");			
				...
				setTimeout(function() { 
					$console_message.html("Creating packages for Rubies");	
					progressBarConsole(58, $('#progressBarConsole'));			
				}, 18000 );
				setTimeout(function() { 
					$console_message.html("Generating <span class='console-message-connected-item'>" + $console_message_Rubies_msg + "</span> Rubies");
					$('.console-Rubies-wrapper').fadeIn(500, function() {
						var $console_Rubies_countto = $('#slider-Rubies').slider("option", "value");					
						$('#console-Rubies-value').countTo({
							from: 0,
							to: $console_Rubies_countto,
							speed: 2500,
							refreshInterval: 10,
							formatter: function (value, options) {
							  return value.toFixed(options.decimals);
				...
				setTimeout(function() { 
					$console_message.html("<span class='console-message-connected-item'>" + $console_message_Rubies_msg + "</span> <span class='console-message-success'> Rubies successfully generated</span>");
					$('#console-success-confirmation-Rubies').fadeIn();
					$(".console-message").addClass('pulse animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){
						$(this).removeClass('pulse animated');
					});
				...
				setTimeout(function() {
					...
					$('#instructions-completion-indicator-step-3').addClass('step-pending').html( "<i class='fa fa-repeat fa-Rubies' aria-hidden='true'></i>" );
					if ($(window).width() < 600) {
