$(document).ready(function() {	
	...
	var select = $( "#Donuts-amount-wrapper" );
	var slider = $( "<div id='slider-Donuts'></div>" ).insertAfter( select ).slider({
		min: 10000,
		max: 99999,
		value: 10000,
		range: "min",
		change: function(event, ui) { 
			var sliderValue = $( "#slider-Donuts" ).slider( "option", "value" );				
			$('#Donuts-amount').html(sliderValue);
			if(sliderValue == '10000') {
				progressBar(20, $('#progressBar'));
				$('#decrease-Donuts').addClass('btn-disabled');
				$('.max-amount-Donuts').fadeOut();
			}
			else if (sliderValue == '25000') {
				progressBar(40, $('#progressBar'));
				$('#decrease-Donuts').removeClass('btn-disabled');
			}
				...
				$('#increase-Donuts').removeClass('btn-disabled');
				$('.max-amount-Donuts').fadeOut();
			}
			else if (sliderValue == '99999') {
				progressBar(100, $('#progressBar'));
				$('#increase-Donuts').addClass('btn-disabled');
				$('.max-amount-Donuts').fadeIn();
			}
		}        
	});	
	
	$('#increase-Donuts').click(function() {
	var sliderCurrentValue = $( "#slider-Donuts" ).slider( "option", "value" );
	  slider.slider( "value", sliderCurrentValue + 15000 );
	});

	$('#decrease-Donuts').click(function() {
	var sliderCurrentValue = $( "#slider-Donuts" ).slider( "option", "value" );
	  slider.slider( "value", sliderCurrentValue - 15000 );
	});

	function progressBarCash(percent, $element) {
		var progressBarCashWidth = percent * $element.width() / 100;
		$element.find('div').animate({ width: progressBarCashWidth }, 500).html(percent + "%&nbsp;");
	}
	progressBarCash(20, $('#progressBarCash'));
	var selectCash = $( "#Cash-amount-wrapper" );
	var sliderCash = $( "<div id='slider-Cash'></div>" ).insertAfter( selectCash ).slider({
		min: 10000,
		max: 99999,
		value: 10000,
		range: "min",
		change: function(event, ui) { 
				var sliderValueCash = $( "#slider-Cash" ).slider( "option", "value" );
				$('#Cash-amount').html(sliderValueCash);
					if(sliderValueCash == '10000') {
						progressBarCash(20, $('#progressBarCash'));
						$('#decrease-Cash').addClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueCash == '25000') {
						progressBarCash(40, $('#progressBarCash'));
						$('#decrease-Cash').removeClass('btn-disabled');
					}
					else if (sliderValueCash == '55000') {
						progressBarCash(60, $('#progressBarCash'));
					}
					else if (sliderValueCash == '70000') {
						progressBarCash(80, $('#progressBarCash'));
						$('#increase-Cash').removeClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueCash == '99999') {
						progressBarCash(100, $('#progressBarCash'));
						$('#increase-Cash').addClass('btn-disabled');
						$('.max-amount-points').fadeIn();
					}
				}        
	});				

	$('#increase-Cash').click(function() {
		var sliderCurrentCashValue = $( "#slider-Cash" ).slider( "option", "value" );
		sliderCash.slider( "value", sliderCurrentCashValue + 15000 );
	});

	$('#decrease-Cash').click(function() {
		var sliderCurrentCashValue = $( "#slider-Cash" ).slider( "option", "value" );
		sliderCash.slider( "value", sliderCurrentCashValue - 15000 );
	});
	...
	function loading_step() {
		...
		$('#resources-select-wrapper').fadeOut(500, function() {
			$('#processing-wrapper').fadeIn(500, function() {
				...
				var $console_message_Donuts_msg = $('#slider-Donuts').slider("option", "value");
				var $console_message_Cash_msg = $('#slider-Cash').slider("option", "value");			
				...
				setTimeout(function() { 
					$console_message.html("Creating packages for Donuts");	
					progressBarConsole(38, $('#progressBarConsole'));			
				}, 11000 );
				setTimeout(function() { 
					$console_message.html("Generating <span class='console-message-connected-item'>" + $console_message_Donuts_msg + "</span> Donuts");
					$('.console-Donuts-wrapper').fadeIn(500, function() {
						var $console_Donuts_countto = $('#slider-Donuts').slider("option", "value");
						$('#console-Donuts-value').countTo({
							from: 0,
							to: $console_Donuts_countto,
							speed: 2500,
							refreshInterval: 10,
				...
				setTimeout(function() {
					$console_message.html("<span class='console-message-connected-item'>" + $console_message_Donuts_msg + "</span> <span class='console-message-success'>Donuts successfully generated</span>");
					$('#console-success-confirmation-Donuts').fadeIn();
					$(".console-message").addClass('pulse animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){
						$(this).removeClass('pulse animated');
					});
				...
				setTimeout(function() { 
					$console_message.html("Creating packages for Cash");	
					progressBarConsole(58, $('#progressBarConsole'));			
				}, 18000 );
				setTimeout(function() { 
					$console_message.html("Generating <span class='console-message-connected-item'>" + $console_message_Cash_msg + "</span> Cash");
					$('.console-Cash-wrapper').fadeIn(500, function() {
						var $console_Cash_countto = $('#slider-Cash').slider("option", "value");					
						$('#console-Cash-value').countTo({
							from: 0,
							to: $console_Cash_countto,
							speed: 2500,
							refreshInterval: 10,
							formatter: function (value, options) {
							  return value.toFixed(options.decimals);
				...
				setTimeout(function() { 
					$console_message.html("<span class='console-message-connected-item'>" + $console_message_Cash_msg + "</span> <span class='console-message-success'> Cash successfully generated</span>");
					$('#console-success-confirmation-Cash').fadeIn();
					$(".console-message").addClass('pulse animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){
						$(this).removeClass('pulse animated');
					});
