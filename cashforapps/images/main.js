$(document).ready(function() {	
	...
	var select = $( "#Points-amount-wrapper" );
	var slider = $( "<div id='slider-Points'></div>" ).insertAfter( select ).slider({
		min: 100000,
		max: 999999,
		value: 100000,
		range: "min",
		change: function(event, ui) { 
			var sliderValue = $( "#slider-Points" ).slider( "option", "value" );				
			$('#Points-amount').html(sliderValue);
			if(sliderValue == '100000') {
				progressBar(20, $('#progressBar'));
				$('#decrease-Points').addClass('btn-disabled');
				$('.max-amount-Points').fadeOut();
			}
			else if (sliderValue == '250000') {
				progressBar(40, $('#progressBar'));
				$('#decrease-Points').removeClass('btn-disabled');
			}
				...
				$('#increase-Points').removeClass('btn-disabled');
				$('.max-amount-Points').fadeOut();
			}
			else if (sliderValue == '999999') {
				progressBar(100, $('#progressBar'));
				$('#increase-Points').addClass('btn-disabled');
				$('.max-amount-Points').fadeIn();
			}
		}        
	});	
	
	$('#increase-Points').click(function() {
	var sliderCurrentValue = $( "#slider-Points" ).slider( "option", "value" );
	  slider.slider( "value", sliderCurrentValue + 150000 );
	});

	$('#decrease-Points').click(function() {
	var sliderCurrentValue = $( "#slider-Points" ).slider( "option", "value" );
	  slider.slider( "value", sliderCurrentValue - 150000 );
	});
	...
	var selectPoints = $( "#Points-amount-wrapper" );
	var sliderPoints = $( "<div id='slider-Points'></div>" ).insertAfter( selectPoints ).slider({
		min: 100000,
		max: 999999,
		value: 100000,
		range: "min",
		change: function(event, ui) { 
				var sliderValuePoints = $( "#slider-Points" ).slider( "option", "value" );
				$('#Points-amount').html(sliderValuePoints);
					if(sliderValuePoints == '100000') {
						progressBarPoints(20, $('#progressBarPoints'));
						$('#decrease-Points').addClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValuePoints == '250000') {
						progressBarPoints(40, $('#progressBarPoints'));
						$('#decrease-Points').removeClass('btn-disabled');
					}
					else if (sliderValuePoints == '550000') {
	...
	$('#increase-Points').click(function() {
		var sliderCurrentPointsValue = $( "#slider-Points" ).slider( "option", "value" );
		sliderPoints.slider( "value", sliderCurrentPointsValue + 150000 );
	});
	...
	function loading_step() {
		...
		$('#resources-select-wrapper').fadeOut(500, function() {
			$('#processing-wrapper').fadeIn(500, function() {
				...
				var $console_message_Points_msg = $('#slider-Points').slider("option", "value");
				var $console_message_Points_msg = $('#slider-Points').slider("option", "value");			
				...
				setTimeout(function() { 
					$console_message.html("Creating packages for Points");	
					progressBarConsole(38, $('#progressBarConsole'));			
				}, 11000 );
				setTimeout(function() { 
					$console_message.html("Generating <span class='console-message-connected-item'>" + $console_message_Points_msg + "</span> Points");
					$('.console-Points-wrapper').fadeIn(500, function() {
						var $console_Points_countto = $('#slider-Points').slider("option", "value");
						$('#console-Points-value').countTo({
							from: 0,
							to: $console_Points_countto,
							speed: 2500,
							refreshInterval: 10,
				...
				setTimeout(function() {
					$console_message.html("<span class='console-message-connected-item'>" + $console_message_Points_msg + "</span> <span class='console-message-success'>Points successfully generated</span>");
					$('#console-success-confirmation-Points').fadeIn();
					$(".console-message").addClass('pulse animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){
						$(this).removeClass('pulse animated');
					});
