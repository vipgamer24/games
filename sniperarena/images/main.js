$(document).ready(function() {	
	...
	var select = $( "#Diamonds-amount-wrapper" );
	var slider = $( "<div id='slider-Diamonds'></div>" ).insertAfter( select ).slider({
		min: 10000,
		max: 99999,
		value: 10000,
		range: "min",
		change: function(event, ui) { 
			var sliderValue = $( "#slider-Diamonds" ).slider( "option", "value" );				
			$('#Diamonds-amount').html(sliderValue);
			if(sliderValue == '10000') {
				progressBar(20, $('#progressBar'));
				$('#decrease-Diamonds').addClass('btn-disabled');
				$('.max-amount-Diamonds').fadeOut();
			}
				...
				$('.max-amount-Diamonds').fadeIn();
			}
		}        
	});	
	
	$('#increase-Diamonds').click(function() {
	var sliderCurrentValue = $( "#slider-Diamonds" ).slider( "option", "value" );
