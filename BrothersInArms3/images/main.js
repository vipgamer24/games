$(document).ready(function() {	
	...
	var select = $( "#Medals-amount-wrapper" );
	var slider = $( "<div id='slider-Medals'></div>" ).insertAfter( select ).slider({
		min: 100000,
		max: 999999,
		value: 100000,
		range: "min",
		change: function(event, ui) { 
			var sliderValue = $( "#slider-Medals" ).slider( "option", "value" );				
			$('#Medals-amount').html(sliderValue);
			if(sliderValue == '100000') {
				progressBar(20, $('#progressBar'));
				$('#decrease-Medals').addClass('btn-disabled');
				$('.max-amount-Medals').fadeOut();
			}
			else if (sliderValue == '250000') {
				progressBar(40, $('#progressBar'));
				$('#decrease-Medals').removeClass('btn-disabled');
			}
				...
				$('#increase-Medals').removeClass('btn-disabled');
				$('.max-amount-Medals').fadeOut();
			}
			else if (sliderValue == '999999') {
				progressBar(100, $('#progressBar'));
				$('#increase-Medals').addClass('btn-disabled');
				$('.max-amount-Medals').fadeIn();
			}
		}        
	});	
	
	$('#increase-Medals').click(function() {
	var sliderCurrentValue = $( "#slider-Medals" ).slider( "option", "value" );
	  slider.slider( "value", sliderCurrentValue + 150000 );
	});

	$('#decrease-Medals').click(function() {
	var sliderCurrentValue = $( "#slider-Medals" ).slider( "option", "value" );
	  slider.slider( "value", sliderCurrentValue - 150000 );
	});

	function progressBarMedals(percent, $element) {
		var progressBarMedalsWidth = percent * $element.width() / 100;
		$element.find('div').animate({ width: progressBarMedalsWidth }, 500).html(percent + "%&nbsp;");
	}
	progressBarMedals(20, $('#progressBarMedals'));
	var selectMedals = $( "#Medals-amount-wrapper" );
	var sliderMedals = $( "<div id='slider-Medals'></div>" ).insertAfter( selectMedals ).slider({
		min: 100000,
		max: 999999,
		value: 100000,
		range: "min",
		change: function(event, ui) { 
				var sliderValueMedals = $( "#slider-Medals" ).slider( "option", "value" );
				$('#Medals-amount').html(sliderValueMedals);
					if(sliderValueMedals == '100000') {
						progressBarMedals(20, $('#progressBarMedals'));
						$('#decrease-Medals').addClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueMedals == '250000') {
						progressBarMedals(40, $('#progressBarMedals'));
						$('#decrease-Medals').removeClass('btn-disabled');
					}
					else if (sliderValueMedals == '550000') {
						progressBarMedals(60, $('#progressBarMedals'));
					}
					else if (sliderValueMedals == '700000') {
						progressBarMedals(80, $('#progressBarMedals'));
						$('#increase-Medals').removeClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueMedals == '999999') {
						progressBarMedals(100, $('#progressBarMedals'));
						$('#increase-Medals').addClass('btn-disabled');
	...
	$('#increase-Medals').click(function() {
		var sliderCurrentMedalsValue = $( "#slider-Medals" ).slider( "option", "value" );
		sliderMedals.slider( "value", sliderCurrentMedalsValue + 150000 );
	});
	...
	function loading_step() {
		...
		$('#resources-select-wrapper').fadeOut(500, function() {
			$('#processing-wrapper').fadeIn(500, function() {
				...
				var $console_message_Medals_msg = $('#slider-Medals').slider("option", "value");
				var $console_message_Medals_msg = $('#slider-Medals').slider("option", "value");			
				...
				setTimeout(function() { 
					$console_message.html("Creating packages for Medals");	
					progressBarConsole(38, $('#progressBarConsole'));			
				}, 11000 );
				setTimeout(function() { 
					$console_message.html("Generating <span class='console-message-connected-item'>" + $console_message_Medals_msg + "</span> Medals");
					$('.console-Medals-wrapper').fadeIn(500, function() {
						var $console_Medals_countto = $('#slider-Medals').slider("option", "value");
						$('#console-Medals-value').countTo({
							from: 0,
							to: $console_Medals_countto,
							speed: 2500,
							refreshInterval: 10,
				...
				setTimeout(function() {
					$console_message.html("<span class='console-message-connected-item'>" + $console_message_Medals_msg + "</span> <span class='console-message-success'>Medals successfully generated</span>");
					$('#console-success-confirmation-Medals').fadeIn();
					$(".console-message").addClass('pulse animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){
						$(this).removeClass('pulse animated');
					});
