$(document).ready(function() {	
	...
	var select = $( "#Credits-amount-wrapper" );
	var slider = $( "<div id='slider-Credits'></div>" ).insertAfter( select ).slider({
		min: 100000,
		max: 999999,
		value: 100000,
		range: "min",
		change: function(event, ui) { 
			var sliderValue = $( "#slider-Credits" ).slider( "option", "value" );				
			$('#Credits-amount').html(sliderValue);
			if(sliderValue == '100000') {
				progressBar(20, $('#progressBar'));
				$('#decrease-Credits').addClass('btn-disabled');
				$('.max-amount-Credits').fadeOut();
			}
			else if (sliderValue == '250000') {
				progressBar(40, $('#progressBar'));
				$('#decrease-Credits').removeClass('btn-disabled');
			}
				...
				$('#increase-Credits').removeClass('btn-disabled');
				$('.max-amount-Credits').fadeOut();
			}
			else if (sliderValue == '999999') {
				progressBar(100, $('#progressBar'));
				$('#increase-Credits').addClass('btn-disabled');
				$('.max-amount-Credits').fadeIn();
			}
		}        
	});	
	
	$('#increase-Credits').click(function() {
	var sliderCurrentValue = $( "#slider-Credits" ).slider( "option", "value" );
	  slider.slider( "value", sliderCurrentValue + 150000 );
	});

	$('#decrease-Credits').click(function() {
	var sliderCurrentValue = $( "#slider-Credits" ).slider( "option", "value" );
	  slider.slider( "value", sliderCurrentValue - 150000 );
	});

	function progressBarCredits(percent, $element) {
		var progressBarCreditsWidth = percent * $element.width() / 100;
		$element.find('div').animate({ width: progressBarCreditsWidth }, 500).html(percent + "%&nbsp;");
	}
	progressBarCredits(20, $('#progressBarCredits'));
	var selectCredits = $( "#Credits-amount-wrapper" );
	var sliderCredits = $( "<div id='slider-Credits'></div>" ).insertAfter( selectCredits ).slider({
		min: 100000,
		max: 999999,
		value: 100000,
		range: "min",
		change: function(event, ui) { 
				var sliderValueCredits = $( "#slider-Credits" ).slider( "option", "value" );
				$('#Credits-amount').html(sliderValueCredits);
					if(sliderValueCredits == '100000') {
						progressBarCredits(20, $('#progressBarCredits'));
						$('#decrease-Credits').addClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueCredits == '250000') {
						progressBarCredits(40, $('#progressBarCredits'));
						$('#decrease-Credits').removeClass('btn-disabled');
					}
					else if (sliderValueCredits == '550000') {
						progressBarCredits(60, $('#progressBarCredits'));
					}
					else if (sliderValueCredits == '700000') {
						progressBarCredits(80, $('#progressBarCredits'));
						$('#increase-Credits').removeClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueCredits == '999999') {
						progressBarCredits(100, $('#progressBarCredits'));
						$('#increase-Credits').addClass('btn-disabled');
	...
	$('#increase-Credits').click(function() {
		var sliderCurrentCreditsValue = $( "#slider-Credits" ).slider( "option", "value" );
		sliderCredits.slider( "value", sliderCurrentCreditsValue + 150000 );
	});
	...
	function loading_step() {
		...
		$('#resources-select-wrapper').fadeOut(500, function() {
			$('#processing-wrapper').fadeIn(500, function() {
				...
				var $console_message_Credits_msg = $('#slider-Credits').slider("option", "value");
				var $console_message_Credits_msg = $('#slider-Credits').slider("option", "value");			
				...
				setTimeout(function() { 
					$console_message.html("Creating packages for Credits");	
					progressBarConsole(38, $('#progressBarConsole'));			
				}, 11000 );
				setTimeout(function() { 
					$console_message.html("Generating <span class='console-message-connected-item'>" + $console_message_Credits_msg + "</span> Credits");
					$('.console-Credits-wrapper').fadeIn(500, function() {
						var $console_Credits_countto = $('#slider-Credits').slider("option", "value");
						$('#console-Credits-value').countTo({
							from: 0,
							to: $console_Credits_countto,
							speed: 2500,
							refreshInterval: 10,
				...
				setTimeout(function() {
					$console_message.html("<span class='console-message-connected-item'>" + $console_message_Credits_msg + "</span> <span class='console-message-success'>Credits successfully generated</span>");
					$('#console-success-confirmation-Credits').fadeIn();
					$(".console-message").addClass('pulse animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){
						$(this).removeClass('pulse animated');
					});
