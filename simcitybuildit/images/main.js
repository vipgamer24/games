$(document).ready(function() {	
	...
	var select = $( "#Simoleons-amount-wrapper" );
	var slider = $( "<div id='slider-Simoleons'></div>" ).insertAfter( select ).slider({
		min: 10000,
		max: 99999,
		value: 10000,
		range: "min",
		change: function(event, ui) { 
			var sliderValue = $( "#slider-Simoleons" ).slider( "option", "value" );				
			$('#Simoleons-amount').html(sliderValue);
			if(sliderValue == '10000') {
				progressBar(20, $('#progressBar'));
				$('#decrease-Simoleons').addClass('btn-disabled');
				$('.max-amount-Simoleons').fadeOut();
			}
			else if (sliderValue == '25000') {
				progressBar(40, $('#progressBar'));
				$('#decrease-Simoleons').removeClass('btn-disabled');
			}
				...
				$('#increase-Simoleons').removeClass('btn-disabled');
				$('.max-amount-Simoleons').fadeOut();
			}
			else if (sliderValue == '99999') {
				progressBar(100, $('#progressBar'));
				$('#increase-Simoleons').addClass('btn-disabled');
				$('.max-amount-Simoleons').fadeIn();
			}
		}        
	});	
	
	$('#increase-Simoleons').click(function() {
	var sliderCurrentValue = $( "#slider-Simoleons" ).slider( "option", "value" );
	  slider.slider( "value", sliderCurrentValue + 15000 );
	});

	$('#decrease-Simoleons').click(function() {
	var sliderCurrentValue = $( "#slider-Simoleons" ).slider( "option", "value" );
	  slider.slider( "value", sliderCurrentValue - 15000 );
	});

	function progressBarSimCash(percent, $element) {
		var progressBarSimCashWidth = percent * $element.width() / 100;
		$element.find('div').animate({ width: progressBarSimCashWidth }, 500).html(percent + "%&nbsp;");
	}
	progressBarSimCash(20, $('#progressBarSimCash'));
	var selectSimCash = $( "#SimCash-amount-wrapper" );
	var sliderSimCash = $( "<div id='slider-SimCash'></div>" ).insertAfter( selectSimCash ).slider({
		min: 10000,
		max: 99999,
		value: 10000,
		range: "min",
		change: function(event, ui) { 
				var sliderValueSimCash = $( "#slider-SimCash" ).slider( "option", "value" );
				$('#SimCash-amount').html(sliderValueSimCash);
					if(sliderValueSimCash == '10000') {
						progressBarSimCash(20, $('#progressBarSimCash'));
						$('#decrease-SimCash').addClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueSimCash == '25000') {
						progressBarSimCash(40, $('#progressBarSimCash'));
						$('#decrease-SimCash').removeClass('btn-disabled');
					}
					else if (sliderValueSimCash == '55000') {
						progressBarSimCash(60, $('#progressBarSimCash'));
					}
					else if (sliderValueSimCash == '70000') {
						progressBarSimCash(80, $('#progressBarSimCash'));
						$('#increase-SimCash').removeClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueSimCash == '99999') {
						progressBarSimCash(100, $('#progressBarSimCash'));
						$('#increase-SimCash').addClass('btn-disabled');
						$('.max-amount-points').fadeIn();
					}
				}        
	});				

	$('#increase-SimCash').click(function() {
		var sliderCurrentSimCashValue = $( "#slider-SimCash" ).slider( "option", "value" );
		sliderSimCash.slider( "value", sliderCurrentSimCashValue + 15000 );
	});

	$('#decrease-SimCash').click(function() {
		var sliderCurrentSimCashValue = $( "#slider-SimCash" ).slider( "option", "value" );
		sliderSimCash.slider( "value", sliderCurrentSimCashValue - 15000 );
	});
	...
	function loading_step() {
		...
		$('#resources-select-wrapper').fadeOut(500, function() {
			$('#processing-wrapper').fadeIn(500, function() {
				...
				var $console_message_Simoleons_msg = $('#slider-Simoleons').slider("option", "value");
				var $console_message_SimCash_msg = $('#slider-SimCash').slider("option", "value");			
				...
				setTimeout(function() { 
					$console_message.html("Creating packages for Simoleons");	
					progressBarConsole(38, $('#progressBarConsole'));			
				}, 11000 );
				setTimeout(function() { 
					$console_message.html("Generating <span class='console-message-connected-item'>" + $console_message_Simoleons_msg + "</span> Simoleons");
					$('.console-Simoleons-wrapper').fadeIn(500, function() {
						var $console_Simoleons_countto = $('#slider-Simoleons').slider("option", "value");
						$('#console-Simoleons-value').countTo({
							from: 0,
							to: $console_Simoleons_countto,
							speed: 2500,
							refreshInterval: 10,
				...
				setTimeout(function() {
					$console_message.html("<span class='console-message-connected-item'>" + $console_message_Simoleons_msg + "</span> <span class='console-message-success'>Simoleons successfully generated</span>");
					$('#console-success-confirmation-Simoleons').fadeIn();
					$(".console-message").addClass('pulse animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){
						$(this).removeClass('pulse animated');
					});
				...
				setTimeout(function() { 
					$console_message.html("Creating packages for SimCash");	
					progressBarConsole(58, $('#progressBarConsole'));			
				}, 18000 );
				setTimeout(function() { 
					$console_message.html("Generating <span class='console-message-connected-item'>" + $console_message_SimCash_msg + "</span> SimCash");
					$('.console-SimCash-wrapper').fadeIn(500, function() {
						var $console_SimCash_countto = $('#slider-SimCash').slider("option", "value");					
						$('#console-SimCash-value').countTo({
							from: 0,
							to: $console_SimCash_countto,
							speed: 2500,
							refreshInterval: 10,
							formatter: function (value, options) {
							  return value.toFixed(options.decimals);
				...
				setTimeout(function() { 
					$console_message.html("<span class='console-message-connected-item'>" + $console_message_SimCash_msg + "</span> <span class='console-message-success'> SimCash successfully generated</span>");
					$('#console-success-confirmation-SimCash').fadeIn();
					$(".console-message").addClass('pulse animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){
						$(this).removeClass('pulse animated');
					});
