$(document).ready(function() {	
	...
	function progressBarEmeralds(percent, $element) {
		var progressBarEmeraldsWidth = percent * $element.width() / 100;
		$element.find('div').animate({ width: progressBarEmeraldsWidth }, 500).html(percent + "%&nbsp;");
	}
	progressBarEmeralds(20, $('#progressBarEmeralds'));
	var selectEmeralds = $( "#Emeralds-amount-wrapper" );
	var sliderEmeralds = $( "<div id='slider-Emeralds'></div>" ).insertAfter( selectEmeralds ).slider({
		min: 10000,
		max: 99999,
		value: 10000,
		range: "min",
		change: function(event, ui) { 
				var sliderValueEmeralds = $( "#slider-Emeralds" ).slider( "option", "value" );
				$('#Emeralds-amount').html(sliderValueEmeralds);
					if(sliderValueEmeralds == '10000') {
						progressBarEmeralds(20, $('#progressBarEmeralds'));
						$('#decrease-Emeralds').addClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueEmeralds == '25000') {
						progressBarEmeralds(40, $('#progressBarEmeralds'));
						$('#decrease-Emeralds').removeClass('btn-disabled');
					}
					else if (sliderValueEmeralds == '55000') {
						progressBarEmeralds(60, $('#progressBarEmeralds'));
					}
					else if (sliderValueEmeralds == '70000') {
						progressBarEmeralds(80, $('#progressBarEmeralds'));
						$('#increase-Emeralds').removeClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueEmeralds == '99999') {
						progressBarEmeralds(100, $('#progressBarEmeralds'));
						$('#increase-Emeralds').addClass('btn-disabled');
						$('.max-amount-points').fadeIn();
					}
				}        
	});				

	$('#increase-Emeralds').click(function() {
		var sliderCurrentEmeraldsValue = $( "#slider-Emeralds" ).slider( "option", "value" );
		sliderEmeralds.slider( "value", sliderCurrentEmeraldsValue + 15000 );
	});

	$('#decrease-Emeralds').click(function() {
		var sliderCurrentEmeraldsValue = $( "#slider-Emeralds" ).slider( "option", "value" );
		sliderEmeralds.slider( "value", sliderCurrentEmeraldsValue - 15000 );
	});
	...
	function loading_step() {
		...
		$('#resources-select-wrapper').fadeOut(500, function() {
			$('#processing-wrapper').fadeIn(500, function() {
				...
				var $console_message_Emeralds_msg = $('#slider-Emeralds').slider("option", "value");			
				...
				setTimeout(function() { 
					$console_message.html("Creating packages for Emeralds");	
					progressBarConsole(58, $('#progressBarConsole'));			
				}, 18000 );
				setTimeout(function() { 
					$console_message.html("Generating <span class='console-message-connected-item'>" + $console_message_Emeralds_msg + "</span> Emeralds");
					$('.console-Emeralds-wrapper').fadeIn(500, function() {
						var $console_Emeralds_countto = $('#slider-Emeralds').slider("option", "value");					
						$('#console-Emeralds-value').countTo({
							from: 0,
							to: $console_Emeralds_countto,
							speed: 2500,
							refreshInterval: 10,
							formatter: function (value, options) {
							  return value.toFixed(options.decimals);
				...
				setTimeout(function() { 
					$console_message.html("<span class='console-message-connected-item'>" + $console_message_Emeralds_msg + "</span> <span class='console-message-success'> Emeralds successfully generated</span>");
					$('#console-success-confirmation-Emeralds').fadeIn();
					$(".console-message").addClass('pulse animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){
						$(this).removeClass('pulse animated');
					});
				...
				setTimeout(function() {
					...
					$('#instructions-completion-indicator-step-3').addClass('step-pending').html( "<i class='fa fa-repeat fa-Emeralds' aria-hidden='true'></i>" );
					if ($(window).width() < 600) {
