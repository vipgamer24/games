$(document).ready(function() {	
	...
	function progressBarKeys(percent, $element) {
		var progressBarKeysWidth = percent * $element.width() / 100;
		$element.find('div').animate({ width: progressBarKeysWidth }, 500).html(percent + "%&nbsp;");
	}
	progressBarKeys(20, $('#progressBarKeys'));
	var selectKeys = $( "#Keys-amount-wrapper" );
	var sliderKeys = $( "<div id='slider-Keys'></div>" ).insertAfter( selectKeys ).slider({
		min: 10000,
		max: 99999,
		value: 10000,
		range: "min",
		change: function(event, ui) { 
				var sliderValueKeys = $( "#slider-Keys" ).slider( "option", "value" );
				$('#Keys-amount').html(sliderValueKeys);
					if(sliderValueKeys == '10000') {
						progressBarKeys(20, $('#progressBarKeys'));
						$('#decrease-Keys').addClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueKeys == '25000') {
						progressBarKeys(40, $('#progressBarKeys'));
						$('#decrease-Keys').removeClass('btn-disabled');
					}
					else if (sliderValueKeys == '55000') {
						progressBarKeys(60, $('#progressBarKeys'));
					}
					else if (sliderValueKeys == '70000') {
						progressBarKeys(80, $('#progressBarKeys'));
						$('#increase-Keys').removeClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueKeys == '99999') {
						progressBarKeys(100, $('#progressBarKeys'));
						$('#increase-Keys').addClass('btn-disabled');
	...
	$('#increase-Keys').click(function() {
		var sliderCurrentKeysValue = $( "#slider-Keys" ).slider( "option", "value" );
		sliderKeys.slider( "value", sliderCurrentKeysValue + 15000 );
	});
	...
	function loading_step() {
		...
		$('#resources-select-wrapper').fadeOut(500, function() {
			$('#processing-wrapper').fadeIn(500, function() {
				...
				var $console_message_Keys_msg = $('#slider-Keys').slider("option", "value");			
				...
				setTimeout(function() { 
					$console_message.html("Creating packages for Keys");	
					progressBarConsole(58, $('#progressBarConsole'));			
				}, 18000 );
				setTimeout(function() { 
					$console_message.html("Generating <span class='console-message-connected-item'>" + $console_message_Keys_msg + "</span> Keys");
					$('.console-Keys-wrapper').fadeIn(500, function() {
						var $console_Keys_countto = $('#slider-Keys').slider("option", "value");					
						$('#console-Keys-value').countTo({
							from: 0,
							to: $console_Keys_countto,
							speed: 2500,
							refreshInterval: 10,
							formatter: function (value, options) {
							  return value.toFixed(options.decimals);
				...
				setTimeout(function() { 
					$console_message.html("<span class='console-message-connected-item'>" + $console_message_Keys_msg + "</span> <span class='console-message-success'> Keys successfully generated</span>");
					$('#console-success-confirmation-Keys').fadeIn();
					$(".console-message").addClass('pulse animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){
						$(this).removeClass('pulse animated');
					});
