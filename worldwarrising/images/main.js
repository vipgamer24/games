$(document).ready(function() {	
	...
	function progressBarGold(percent, $element) {
		var progressBarGoldWidth = percent * $element.width() / 100;
		$element.find('div').animate({ width: progressBarGoldWidth }, 500).html(percent + "%&nbsp;");
	}
	progressBarGold(20, $('#progressBarGold'));
	var selectGold = $( "#Gold-amount-wrapper" );
	var sliderGold = $( "<div id='slider-Gold'></div>" ).insertAfter( selectGold ).slider({
		min: 100000,
		max: 999999,
		value: 100000,
		range: "min",
		change: function(event, ui) { 
				var sliderValueGold = $( "#slider-Gold" ).slider( "option", "value" );
				$('#Gold-amount').html(sliderValueGold);
					if(sliderValueGold == '100000') {
						progressBarGold(20, $('#progressBarGold'));
						$('#decrease-Gold').addClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueGold == '250000') {
						progressBarGold(40, $('#progressBarGold'));
						$('#decrease-Gold').removeClass('btn-disabled');
					}
					else if (sliderValueGold == '550000') {
						progressBarGold(60, $('#progressBarGold'));
					}
					else if (sliderValueGold == '700000') {
						progressBarGold(80, $('#progressBarGold'));
						$('#increase-Gold').removeClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueGold == '999999') {
						progressBarGold(100, $('#progressBarGold'));
						$('#increase-Gold').addClass('btn-disabled');
	...
	$('#increase-Gold').click(function() {
		var sliderCurrentGoldValue = $( "#slider-Gold" ).slider( "option", "value" );
		sliderGold.slider( "value", sliderCurrentGoldValue + 150000 );
	});
