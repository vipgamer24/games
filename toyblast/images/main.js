$(document).ready(function() {	
	...
	function progressBarLives(percent, $element) {
		var progressBarLivesWidth = percent * $element.width() / 100;
		$element.find('div').animate({ width: progressBarLivesWidth }, 500).html(percent + "%&nbsp;");
	}
	progressBarLives(20, $('#progressBarLives'));
	var selectLives = $( "#Lives-amount-wrapper" );
	var sliderLives = $( "<div id='slider-Lives'></div>" ).insertAfter( selectLives ).slider({
		min: 10000,
		max: 99999,
		value: 10000,
		range: "min",
		change: function(event, ui) { 
				var sliderValueLives = $( "#slider-Lives" ).slider( "option", "value" );
				$('#Lives-amount').html(sliderValueLives);
					if(sliderValueLives == '10000') {
						progressBarLives(20, $('#progressBarLives'));
						$('#decrease-Lives').addClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueLives == '25000') {
						progressBarLives(40, $('#progressBarLives'));
						$('#decrease-Lives').removeClass('btn-disabled');
					}
					else if (sliderValueLives == '55000') {
						progressBarLives(60, $('#progressBarLives'));
					}
					else if (sliderValueLives == '70000') {
						progressBarLives(80, $('#progressBarLives'));
						$('#increase-Lives').removeClass('btn-disabled');
						$('.max-amount-points').fadeOut();
					}
					else if (sliderValueLives == '99999') {
						progressBarLives(100, $('#progressBarLives'));
						$('#increase-Lives').addClass('btn-disabled');
						$('.max-amount-points').fadeIn();
					}
				}        
	});				

	$('#increase-Lives').click(function() {
		var sliderCurrentLivesValue = $( "#slider-Lives" ).slider( "option", "value" );
		sliderLives.slider( "value", sliderCurrentLivesValue + 15000 );
	});

	$('#decrease-Lives').click(function() {
		var sliderCurrentLivesValue = $( "#slider-Lives" ).slider( "option", "value" );
		sliderLives.slider( "value", sliderCurrentLivesValue - 15000 );
	});
	...
	function loading_step() {
		...
		$('#resources-select-wrapper').fadeOut(500, function() {
			$('#processing-wrapper').fadeIn(500, function() {
				...
				var $console_message_Lives_msg = $('#slider-Lives').slider("option", "value");			
				...
				setTimeout(function() { 
					$console_message.html("Creating packages for Lives");	
					progressBarConsole(58, $('#progressBarConsole'));			
				}, 18000 );
				setTimeout(function() { 
					$console_message.html("Generating <span class='console-message-connected-item'>" + $console_message_Lives_msg + "</span> Lives");
					$('.console-Lives-wrapper').fadeIn(500, function() {
						var $console_Lives_countto = $('#slider-Lives').slider("option", "value");					
						$('#console-Lives-value').countTo({
							from: 0,
							to: $console_Lives_countto,
							speed: 2500,
							refreshInterval: 10,
							formatter: function (value, options) {
							  return value.toFixed(options.decimals);
				...
				setTimeout(function() { 
					$console_message.html("<span class='console-message-connected-item'>" + $console_message_Lives_msg + "</span> <span class='console-message-success'> Lives successfully generated</span>");
					$('#console-success-confirmation-Lives').fadeIn();
					$(".console-message").addClass('pulse animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){
						$(this).removeClass('pulse animated');
					});
